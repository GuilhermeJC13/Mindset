...
class Cortex(Dispatcher):
    ...
    _events_ = ['inform_error','create_session_done', 'query_profile_done', 'load_unload_profile_done', 
                'save_profile_done', 'get_mc_active_action_done', 'set_mc_active_action_done','mc_brainmap_done', 'mc_action_sensitivity_done', 
    ...
    def open(self):
        ...
        sslopt = {'ca_certs': r"C:\Users\mateu\Documents\Sistemas Embarcados\Isaac\Brain-Controlled-UGV\bci\certificate\rootCA.pem", "cert_reqs": ssl.CERT_REQUIRED}
    ...
    def handle_result(self, recv_dic):
        ...
        if req_id == HAS_ACCESS_RIGHT_ID:
            ...
            if name is None:
                ...
                if name != self.profile_name:
                    warnings.warn("There is profile " + name + " is loaded for headset " + targetheadset_id)
        ...
        elif req_id == SET_MENTAL_COMMAND_ACTIVE_ACTION_ID:
            self.emit('set_mc_active_action_done', data=result_dic)
    ...
    def query_insight_headsets(self):
        print('query insight headsets --------------------------------')
        query_headset_request = {
            "jsonrpc": "2.0", 
            "id": QUERY_HEADSET_ID,
            "method": "queryHeadsets",
