...
class LiveAdvance():
    ...
    def __init__(self, app_client_id, app_client_secret, **kwargs):
        ...
        self.c.bind(set_mc_active_action_done=self.on_set_mc_active_action_done)
    ...
    def start(self, profile_name, headsetId=''):
        ...
        (3) get MC active action -> get MC sensitivity
    ...
    def set_active_actions(self, actions):
        """
        To set active actions for the mental command detection.
        Maximum 4 mental command actions are actived. This doesn't include "neutral"

        Parameters
        ----------
        actions : list, required
            mental command actions to be detected
    ...
    def on_get_sensitiviy_done(self, *args, **kwargs):
        data = kwargs.get('data')
        print('on_get_sensitivity_done: {}'.format(data))
        # subscribe mental command data
        stream = ['com']
        self.subscribe_data(stream)
    
    def on_set_mc_active_action_done(self, *args, **kwargs):
        data = kwargs.get('data')
        print('on_set_mc_active_action_done: {}'.format(data))
        self.set_sensitivity(self.profile_name, [8, 7, 7, 6])
